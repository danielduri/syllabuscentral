...
export function NewModel(props) {
    ...
    const getOptionFromRawName = (parsedName, options) => {
        let i = 0;
        const rawName = parsedName.toLowerCase()
            .replaceAll(" ", "")
            .replaceAll("á", "a")
            .replaceAll("é", "e")
            .replaceAll("í", "i")
        ...
        while (i < options.length){
            if(options[i].value===rawName){
                return options[i]
            }
            i++;
        }

        return generateOptionWithValue(parsedName, rawName);
    }

    const getOptionFromName = (parsedName, options) => {
        let i = 0;

        while (i < options.length){
            if(options[i].value===parsedName){
                return options[i]
            }
            i++;
        }
    ...
    function loadModel() {
        if (props.model !== undefined) {
            ...
            setDegree(getOptionFromRawName(model.degree, degreeOptions));
            setYear(generateOptionWithValue(`${model.year}º`, model.year))
            setPeriod(model.period)
            setLanguage(generateOption(model.language))
            setCode(model.code)
            setCourseName(model.name)
            setIntlName(model.intlName)
            setShorthand(model.shorthand)
            setType(generateOption(model.type))
            setECTS(generateOption(model.ECTS))
            setSubject(generateOption(model.subject))
            setModule(generateOption(model.module))
            //TODO FIX
    ...
    useEffect(() => {
        ...
        if(props.show && props.model!==undefined){
